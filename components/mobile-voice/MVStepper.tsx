...
export default function MVStepper({
  active,
  onStepClick,
  maxReached,
}: {
  ...
  return (
    <div>
        ...
        {STEPS.map((label, i) => {
          ...
          return (
            ...
            >
              ...
              <div
                className={`${on ? "text-[#1C1232]" : "text-[#8A84A3]"} text-center transition-all ${isClickable ? "group-hover:text-[#1C1232]" : ""}`}
              >
      ...
      <div className="md:hidden mb-2 flex items-center justify-between">
        <span className="text-[13px] font-semibold text-[#2F2151]">
          Step {a} of {STEPS.length}
        </span>
      ...
      <div className="mt-3 h-2 w-full rounded-full bg-[#E6E4EC]">
        <div
          className="h-full rounded-full bg-[#2F2151]"
          style={{ width: `${pct}%` }}
        />
      </div>
    </div>
