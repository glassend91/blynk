...
export default function MobileVoiceSignup2({ onNext, onBack, onClose }:{
        ...
        <div className="mx-auto mt-8 max-w-[720px] space-y-4">
          {numbers.map((n, i) => (
            ...
            >
              ...
              <input type="radio" name="mv-number" defaultChecked={i === 0} className="sr-only" />
              <span
                className={[
                  "grid h-5 w-5 place-items-center rounded-full border",
                  i === 0 ? "border-[#5C3B86]" : "border-[#CFC8DA]",
                ].join(" ")}
              >
                <span className={["h-2.5 w-2.5 rounded-full", i === 0 ? "bg-[#5C3B86]" : "bg-transparent"].join(" ")} />
              </span>
            </label>
          ))}
        </div>
