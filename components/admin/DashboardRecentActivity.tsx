"use client";
export default function DashboardRecentActivity({ items }: { items: { title: string; by: string; time: string }[] }) {
  return (
      ...
      <ul className="space-y-3">
        {items?.slice(0, 4)?.map((r, i) => (
          <li key={i} className="flex items-center justify-between rounded-[10px] bg-[#F8F8F8] px-4 py-3">
            ...
            <span className="text-[12px] text-[#6F6C90]">{r.time.split('T')[0]}</span>
          </li>
        ))}
      </ul>
    </div>
