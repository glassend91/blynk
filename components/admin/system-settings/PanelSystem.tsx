...
export default function PanelSystem() {
  ...
  const fetchSettings = async () => {
    try {
      ...
      if (data?.success && data.data) {
        ...
        if (settings.system) {
          setCompany(settings.system.companyName || 'Your Telecommunications Provider');
  ...
  const handleSave = async () => {
    try {
      ...
      const { data } = await apiClient.put<{ success: boolean; message: string }>('/system-settings/system', {
        companyName: company,
        supportEmail: email,
      ...
      if (data?.success) {
        setMessage({ type: 'success', text: data.message || 'System settings saved successfully' });
      }
    } catch (err: any) {
