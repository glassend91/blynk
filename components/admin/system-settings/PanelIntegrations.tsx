...
export default function PanelIntegrations() {
    ...
    try {
      ...
      if (data?.success && data.data) {
        ...
        if (settings.integrations?.connectTel) {
          ...
          setCtTokenUpdatedAt(settings.integrations.connectTel.tokenUpdatedAt || null);
  ...
  const handleSave = async () => {
    try {
      ...
      const res1 = await apiClient.put('/system-settings/integrations', payload);
      const res2 = await apiClient.put('/system-settings/integrations/connect-tel', ctPayload);

      if (res1.data?.success && res2.data?.success) {
        setMessage({ type: 'success', text: 'All integration settings saved successfully' });
        if (ovKey) { setOvHasKey(true); setOvKey(''); }
        if (zohoKey) { setZohoHasKey(true); setZohoKey(''); }
        if (ctPassword) { setCtHasPassword(true); setCtPassword(''); }
      }
    } catch (err: any) {
  ...
  const handleCtLogin = async () => {
    try {
      setCtLoggingIn(true);
      setMessage(null);
      const { data } = await apiClient.post<{ success: boolean; message: string; token: string; tokenUpdatedAt: string }>('/system-settings/integrations/connect-tel/login', {
        email: ctEmail,
        password: ctPassword,
        tenantId: ctTenantId
      });
      if (data?.success) {
        ...
        setMessage({ type: 'success', text: data.message || 'Authenticated with ConnectTel successfully' });
      }
    } catch (err: any) {
      setMessage({ type: 'error', text: err?.response?.data?.message || err?.message || 'Failed to login to ConnectTel' });
    } finally {
  ...
  const handleSaveTenantId = async () => {
    try {
      setSaving(true);
      setMessage(null);
      const { data } = await apiClient.put<{ success: boolean; message: string }>('/system-settings/integrations/connect-tel', {
        tenantId: ctTenantId
      });
      if (data?.success) {
      ...
      setMessage({ type: 'error', text: err?.response?.data?.message || err?.message || 'Failed to save Tenant ID' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="space-y-6">
      {message && (
        <div className={`flex items-center justify-between rounded-[10px] border px-4 py-3 text-[13px] ${message.type === 'success' ? 'border-[#C6F6D5] bg-[#F0FFF4] text-[#22543D]' : 'border-[#FCD1D2] bg-[#FFF5F5] text-[#C53030]'
          }`}>
      ...
      <div className="grid gap-6 md:grid-cols-2">
        ...
        <section className="col-span-full rounded-[12.75px] border border-[#DFDBE3] p-6 bg-purple-50/30">
          ...
          <div className="grid gap-6 md:grid-cols-2">
            <div className="space-y-4">
              <h4 className="text-[14px] font-bold text-[#401B60]">1. Authentication Credentials</h4>
              <div className="grid gap-4">
                <Field label="Email Address">
                  <input value={ctEmail} onChange={e => setCtEmail(e.target.value)} className="w-full rounded-[10px] border border-[#DFDBE3] bg-white px-3 py-[11px] outline-none" />
                </Field>
                <Field label="Password">
                  <input type="password" value={ctPassword} onChange={e => setCtPassword(e.target.value)} placeholder={ctHasPassword ? "••••••••" : "Enter password"} className="w-full rounded-[10px] border border-[#DFDBE3] bg-white px-3 py-[11px] outline-none" />
                </Field>
                <button
                  onClick={handleCtLogin}
                  disabled={ctLoggingIn || !ctEmail || !ctTenantId}
            ...
            <div className="space-y-4">
              <h4 className="text-[14px] font-bold text-[#401B60]">2. Tenant Configuration</h4>
              <div className="grid gap-4">
                <Field label="Tenant ID">
                  <input value={ctTenantId} onChange={e => setCtTenantId(e.target.value)} className="w-full rounded-[10px] border border-[#DFDBE3] bg-white px-3 py-[11px] outline-none" />
                </Field>
                <button
        ...
        <section className="rounded-[12.75px] border border-[#DFDBE3] p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={handleTestOneview} disabled={testing} className="rounded-[6px] bg-[#401B60] px-4 py-[9px] text-[14px] font-semibold text-white disabled:opacity-50">
              {testing ? 'Testing...' : 'Test Connection'}
            </button>
          </div>
          <Field label="API Endpoint">
            <input value={ovApi} onChange={e => setOvApi(e.target.value)} className="w-full rounded-[10px] border border-[#DFDBE3] bg-[#F8F8F8] px-3 py-[11px] outline-none" />
          </Field>
          <div className="mt-4">
            <Field label="API Key">
              <input type="password" value={ovKey || (ovHasKey ? '•••••••••••••' : '')} onChange={e => setOvKey(e.target.value)} placeholder={ovHasKey ? "Enter new API key to update" : "Enter API key"} className="w-full rounded-[10px] border border-[#DFDBE3] bg-[#F8F8F8] px-3 py-[11px] outline-none" />
            </Field>
          </div>
          <div className="mt-5 flex items-center justify-between">
        ...
        <section className="rounded-[12.75px] border border-[#DFDBE3] p-6">
          <div className="mb-4 flex items-center justify-between">
            ...
            <button className="rounded-[6px] bg-[#401B60] px-4 py-[9px] text-[14px] font-semibold text-white">Authorize Zoho</button>
          </div>
          <Field label="Client ID">
            <input value={zohoClient} onChange={e => setZohoClient(e.target.value)} className="w-full rounded-[10px] border border-[#DFDBE3] bg-[#F8F8F8] px-3 py-[11px] outline-none" />
          </Field>
          <div className="mt-4">
            <Field label="API Key">
              <input type="password" value={zohoKey || (zohoHasKey ? '•••••••••••••' : '')} onChange={e => setZohoKey(e.target.value)} placeholder={zohoHasKey ? "Enter new API key to update" : "Enter API key"} className="w-full rounded-[10px] border border-[#DFDBE3] bg-[#F8F8F8] px-3 py-[11px] outline-none" />
            </Field>
          </div>
          <div className="mt-5 flex items-center justify-between">
      ...
      <div className="flex justify-end">
        <button onClick={handleSave} disabled={saving} className="rounded-[6px] bg-[#401B60] px-4 py-[9px] text-[14px] font-semibold text-white disabled:opacity-50">
...
function Switch({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) {
  return (
    <button type="button" onClick={() => onChange(!checked)} className={['h-6 w-11 rounded-full px-1 transition-colors', checked ? 'bg-[#2BBF5A]' : 'bg-[#DFDBE3]'].join(' ')} aria-pressed={checked}>
      <span className={['block h-4 w-4 rounded-full bg-white transition-transform', checked ? 'translate-x-5' : 'translate-x-0'].join(' ')} />
    </button>
  );
}
