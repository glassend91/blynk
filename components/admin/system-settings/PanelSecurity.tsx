...
export default function PanelSecurity() {
  ...
  const handleSave = async () => {
    try {
      ...
      const { data } = await apiClient.put<{ success: boolean; message: string }>('/system-settings/security', {
      ...
      if (data?.success) {
        setMessage({ type: 'success', text: data.message || 'Security settings saved successfully' });
      }
    } catch (err: any) {
        ...
        <div className="rounded-[10px] bg-[#F8F8F8] p-4">
          ...
          {passwordExpiry && (
            <div className="mt-3 flex items-center gap-3">
              <span className="text-[14px] text-[#0A0A0A]">Expiry period (days)</span>
              <input
                type="number"
                min={1}
                max={365}
                value={passwordExpiryDays}
                onChange={(e) => setPasswordExpiryDays(parseInt(e.target.value || '90', 10))}
