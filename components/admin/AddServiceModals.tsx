...
export function NBNServiceModal({
  ...
  const fetchAvailableServices = async (): Promise<
    Array<{
      id: string;
      name: string;
      serviceType: string;
  ...
  const handleSubmit = async () => {
    ...
    const actualCustomerId =
      customerId && customerId !== "1"
        ? customerId
        : customers.find((c) => (c.id || c.userId) === customerId)?.userId ||
          customerId;
    ...
    <ModalShell onClose={onClose} size="wide">
      <SectionPanel>
        <div className="mx-auto max-w-[760px]">
          <h2 className="text-[24px] font-bold text-[#0A0A0A] mb-6">
            Add NBN Service
          </h2>
              ...
              <FormField label="Select Plan *">
                {loadingServices ? (
                  <div className="text-center py-8 text-[#6F6C90]">
                    <svg
                      className="animate-spin h-6 w-6 mx-auto mb-2 text-[#401B60]"
            ...
            <div className="space-y-4">
              ...
              <div className="rounded-[10px] border border-[#DFDBE3] bg-[#F8F8F8] p-4">
                <label className="flex items-start gap-3 cursor-pointer">
                  ...
                  />
                  <span className="text-[14px] text-[#0A0A0A]">
                    <span className="font-semibold text-red-600">*</span> Have
                    you verified the plan details and price with the customer
                    and directed them to our terms of service and terms of
                    conditions?
                  </span>
                </label>
              </div>
...
export function BusinessNBNServiceModal({
  ...
  > => {
    try {
      ...
      if (data?.success && data.data) {
        const businessNbnServices = data.data.filter(
          (s) => s.serviceType === "Business NBN",
        );
...
export function MobileVoiceServiceModal({
  ...
  > => {
    try {
      ...
      if (data?.success && data.data) {
        const mobileVoiceServices = data.data.filter(
          (s) =>
  ...
  const fetchAvailableNumbers = async () => {
    try {
      ...
      setError(null);
      console.log(
        "Fetching available numbers from /api/v1/mobile/reserve/numbers",
      );
  ...
  const handleSubmit = async () => {
    ...
    if (simType === "eSim") {
      ...
      const customer = customers.find(
        (c) => (c.id || c.userId) === actualCustomerId,
      );
    ...
    try {
      ...
      const defaultEmail = customer?.email || "";
      const finalEsimEmail =
        simType === "eSim"
          ? esimNotificationEmail?.trim() || defaultEmail
          : undefined;
              ...
              {simType === "eSim" && (
                <FormField label="eSIM Notification Email *">
                  <input
                    ...
                    placeholder={(() => {
                      const customer = customers.find(
                        (c) => (c.id || c.userId) === customerId,
                      );
                      return (
                        customer?.email || "Enter email for eSIM notifications"
                      );
                    })()}
                  />
                  <p className="mt-1 text-xs text-[#6F6C90]">
                ...
                <FormField label="Select Number *">
                  <div className="space-y-2">
                    <button
                      ...
                      onClick={async () => {
                        console.log(
                          "Button clicked, availableNumbers.length:",
                    ...
                    >
                      <span>
                        {selectedNumber ||
                          "Click to select from available numbers"}
                      </span>
                      {loading ? (
                        <svg
                          className="animate-spin h-4 w-4 text-[#401B60]"
                          xmlns="http://www.w3.org/2000/svg"
                          fill="none"
                          viewBox="0 0 24 24"
                        >
                    ...
                    {showNumbersList && availableNumbers.length > 0 && (
                      <div className="mt-2 max-h-[200px] overflow-y-auto space-y-1 border border-[#DFDBE3] rounded-[10px] p-2 bg-white">
                        <div className="text-[12px] font-semibold text-[#6F6C90] mb-2 px-2">
                        ...
                        {availableNumbers.map((num) => (
                          <button
                            ...
                            className={`w-full text-left p-2 rounded-[6px] text-[13px] transition-colors ${
                              selectedNumber === num
...
export function MobileBroadbandServiceModal({
  ...
  > => {
    try {
      ...
      if (data?.success && data.data) {
        const mobileBroadbandServices = data.data.filter(
          (s) =>
        ...
        console.log(
          "Fetched Mobile Broadband services:",
          mobileBroadbandServices,
        );
