            ...
            <div className="flex items-center justify-center space-x-4">
                {['services', 'addons', 'payment', 'review'].map((stepName, index) => (
                    <div key={stepName} className="flex items-center">
                        <div className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-medium ${step === stepName ? 'bg-[#3F205F] text-white' :
                            ['services', 'addons', 'payment', 'review'].indexOf(step) > index ? 'bg-green-500 text-white' :
                                'bg-gray-200 text-gray-600'
                            }`}>
                            {index + 1}
                        </div>
                        {index < 3 && (
                            <div className={`w-12 h-1 mx-2 ${['services', 'addons', 'payment', 'review'].indexOf(step) > index ? 'bg-green-500' : 'bg-gray-200'
                                }`} />
                        )}
                    </div>
                ))}
                        ...
                        {services.map((service) => (
                            ...
                            >
                                <div className="flex items-center justify-between">
                                    <div>
                                        <h4 className="font-semibold text-[#0A0A0A]">{service.serviceName}</h4>
                                        <p className="text-sm text-gray-600">{service.serviceType}</p>
                                        <p className="text-sm text-gray-500">{service.description}</p>
                                    </div>
                                    <div className="text-right">
                                        <p className="font-semibold text-[#3F205F]">${service.price.toFixed(2)}/month</p>
                                        <p className="text-xs text-gray-500">{service.billingCycle}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
            ...
            {step === 'addons' && selectedService && (
                <div>
                    ...
                    <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                        ...
                        <p className="text-sm text-gray-600">Base price: ${selectedService.price.toFixed(2)}/month</p>
                    </div>
                        ...
                        <div className="space-y-3">
                            {selectedService.addOns.map((addon) => (
                                <div key={addon._id} className="flex items-center justify-between p-3 border border-gray-200 rounded-lg">
                                    ...
                                    <span className="font-semibold text-[#3F205F]">+${addon.price.toFixed(2)}/month</span>
                                </div>
                            ))}
                        </div>
                    ) : (
                        ...
                        <div className="space-y-3">
                            {paymentMethods.map((paymentMethod) => (
                                <div
                                    ...
                                    className={`border rounded-lg p-4 cursor-pointer transition-all ${selectedPaymentMethod === (paymentMethod._id || paymentMethod.id)
                                    ...
                                    <label className="flex items-center justify-between cursor-pointer">
                                        <div className="flex items-center">
                                            <input
                                                ...
                                                checked={selectedPaymentMethod === (paymentMethod._id || paymentMethod.id)}
                                                onChange={(e) => {
                ...
                <div>
                    ...
                    <div className="space-y-4">
                        <div className="p-4 bg-gray-50 rounded-lg">
                            <h4 className="font-semibold">{selectedService.serviceName}</h4>
                            <p className="text-sm text-gray-600">{selectedService.serviceType}</p>
                            <p className="text-sm text-gray-600">Base price: ${selectedService.price.toFixed(2)}/month</p>
                        </div>
                        ...
                        <div className="p-4 bg-[#3F205F] text-white rounded-lg">
                            <div className="flex justify-between items-center">
                                ...
                                <span className="text-xl font-bold">${calculateTotalPrice().toFixed(2)}/month</span>
                            </div>
                        </div>
...
export default function BuyServiceModal({ isOpen, onClose, onSuccess }: BuyServiceModalProps) {
