...
export default function Stepper({ active, onStepClick, maxReached }: { active: number; onStepClick?: (step: number) => void; maxReached?: number }) {
    ...
    <div className="mb-5 sm:mb-6 md:mb-7">
      <div className="hidden items-start justify-between sm:flex">
        {LABELS.map((label, i) => {
          ...
          const isClickable = (!!maxReached ? (n <= maxReached && n !== active) : isPrevious) && !!onStepClick;
          ...
          return (
            ...
            >
              ...
              <div className={["text-center transition-all px-1", isActive ? "text-[#3F2E5A] font-semibold" : "text-[#8A84A3]", isClickable ? "group-hover:text-[#3F2E5A]" : ""].join(" ")}>
                <span className="text-[12px] md:text-[13px] lg:text-[14px] leading-tight flex justify-center items-center">{label}</span>
              </div>
            </div>
          );
        })}
