...
export default function BillingHistory({ customerId }: Props) {
  ...
  const getStatusBadge = (status: string) => {
    ...
    return (
      <span
        className={`inline-block px-2.5 py-1 rounded-[6px] text-[12px] font-semibold ${statusInfo.color} ${statusInfo.bg}`}
      >
        {statusInfo.label}
      </span>
    );
  ...
  const handleDownloadPDF = async (
    ...
    try {
      ...
      const response = await fetch(
        `${baseURL}/api/billing/admin/invoices/${invoiceId}/download`,
        {
          method: "GET",
          headers: {
      ...
      <div className="rounded-[14px] border border-[#DFDBE3] bg-white p-6 shadow-[0_37px_37px_rgba(0,0,0,0.05)]">
        ...
        <div className="text-center py-8">
          <p className="text-[14px] text-[#6F6C90]">
            Select a customer to view billing history
          </p>
        </div>
      </div>
    );
            ...
            <table className="w-full border-collapse text-left">
              ...
              <tbody className="text-[14px] text-[#0A0A0A]">
                {invoices.map((invoice) => (
                  ...
                  >
                    <td className="font-mono text-[#401B60]">
