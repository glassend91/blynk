...
export default function CustomerProfile({
    ...
    try {
        ...
        try {
          ...
          if (customerData) {
            const formattedCustomer: Customer = {
              ...
              type:
                customerData.type === "business" || customerData.businessDetails
                  ? "business"
                  : "residential",
              businessDetails:
                ...
                (customerData.type === "business"
                  ? {
                      ...
                      ABN:
                        customerData.businessDetails?.ABN || customerData.ABN,
                      ACN:
                        customerData.businessDetails?.ACN || customerData.ACN,
                      authorizedContacts:
                        customerData.businessDetails?.authorizedContacts || [],
                    }
                  : undefined),
            };
      ...
      if (data?.success && data.data && data.data.length > 0) {
        ...
        let foundCustomer = customerId
          ? data.data.find((c) => (c.id || c.userId) === customerId) ||
            data.data[0]
          : data.data[0];
        ...
        if (
          ...
          (!foundCustomer ||
            (foundCustomer.id !== customerId &&
              foundCustomer.userId !== customerId))
        ) {
          try {
            ...
            if (directData) {
              foundCustomer = {
                ...
                businessName:
                  directData.businessDetails?.businessName ||
                  directData.businessName,
        ...
        const formattedCustomer: Customer = {
          ...
          dateOfBirth: customerData.dateOfBirth,
          serviceAddress:
            customerData.serviceAddress || foundCustomer.serviceAddress,
          billingAddress: customerData.billingAddress,
          type:
            customerData.type === "business" || customerData.businessDetails
              ? "business"
          ...
          businessDetails:
            ...
            (customerData.type === "business"
              ? {
                  businessName:
                    customerData.businessName || foundCustomer.businessName,
                  ABN:
                    customerData.businessDetails?.ABN ||
                    customerData.ABN ||
                    foundCustomer.ABN,
    ...
    try {
      ...
      if (
        ...
        payload.businessDetails = {
          ...
          authorizedContacts:
            updatedCustomer.businessDetails.authorizedContacts,
        };
      ...
      <div className="rounded-[14px] border border-[#DFDBE3] bg-white p-6 shadow-[0_37px_37px_rgba(0,0,0,0.05)]">
        ...
        <div className="text-center py-8">
          <p className="text-[14px] text-[#6F6C90]">
            No customer selected. Search for a customer to view their profile.
          </p>
        </div>
      </div>
    );
  ...
  const primaryName =
    customer.type === "business" && customer.businessDetails?.businessName
        ...
        <div className="space-y-4">
          ...
          {customer.type === "business" && customer.businessDetails && (
            <>
              ...
              {customer.businessDetails.authorizedContacts &&
                customer.businessDetails.authorizedContacts.length > 0 && (
                  <div>
                    <div className="text-[12px] uppercase tracking-wide text-[#6F6C90] mb-2">
                      Authorized Contacts
                    </div>
                    <div className="space-y-2">
                      {customer.businessDetails.authorizedContacts.map(
                        (contact, idx) => (
          ...
          <div>
            ...
            <div className="text-[14px] text-[#0A0A0A] font-mono">
              {customer.id}
            </div>
          </div>
        </div>
...
function EditCustomerModal({
  ...
  const handleSave = async () => {
    ...
    if (!editedCustomer.serviceAddress)
    ...
    if (editedCustomer.type === "business" && editedCustomer.businessDetails) {
      ...
      if (
        editedCustomer.businessDetails.ABN &&
        !validateABN(editedCustomer.businessDetails.ABN)
      ) {
        newErrors.ABN = "ABN must be 11 digits";
      }
      if (
        editedCustomer.businessDetails.ACN &&
        !validateACN(editedCustomer.businessDetails.ACN)
      ) {
  ...
  const updateAuthorizedContact = (
    ...
    if (!editedCustomer.businessDetails) return;
    const contacts = [
      ...(editedCustomer.businessDetails.authorizedContacts || []),
    ];
        ...
        <div className="space-y-4">
              ...
              <>
                <div>
                  ...
                  <input
                    ...
                    className={`w-full rounded-[10px] border px-4 py-3 text-[14px] outline-none ${
                      errors.businessName
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    />
                    {errors.ACN && (
                      <p className="mt-1 text-xs text-red-600">{errors.ACN}</p>
                    )}
                  </div>
                </div>
          ...
          <div>
            ...
            <input
              ...
              className={`w-full rounded-[10px] border px-4 py-3 text-[14px] outline-none ${
                errors.email ? "border-red-300 bg-red-50" : "border-[#DFDBE3]"
              }`}
            />
            {errors.email && (
          ...
          <div>
            ...
            <input
              ...
              className={`w-full rounded-[10px] border px-4 py-3 text-[14px] outline-none ${
                errors.phone ? "border-red-300 bg-red-50" : "border-[#DFDBE3]"
              }`}
            />
            {errors.phone && (
