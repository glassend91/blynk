...
export default function CustomerNotesTab({ initialQuery }: Props) {
  ...
  const handleSearch = async (overrideQuery?: string) => {
    ...
    try {
      ...
      const globalSearchResponse = await apiClient.get<{
        success: boolean;
        data: any[];
        count: number;
      }>(
      ...
      if (
        globalSearchResponse.data?.success &&
        globalSearchResponse.data.data &&
        globalSearchResponse.data.data.length > 0
      ) {
        // Get customer IDs from global search results
        const customerIds = globalSearchResponse.data.data.map(
          (c: any) => c.id || c.userId,
        );

        // Fetch all notes and filter by customer IDs
        const allNotesResponse = await apiClient.get<{
          success: boolean;
          data: Note[];
        ...
        if (allNotesResponse.data?.success && allNotesResponse.data.data) {
          ...
          const filteredNotes = allNotesResponse.data.data.filter(
            (note: Note) => customerIds.includes(note.customerId),
          );
