...
export default function DashboardHome() {
  ...
  return (
    <>
      <h1 className="mb-6 text-[26px] font-bold text-[#0A0A0A]">Overview of your services and account</h1>
      ...
      <div className="grid grid-cols-3 gap-6">
          ...
          <div className="mt-4 space-y-4">
            {activeServices.length === 0 ? (
              ...
              activeServices.map((s: any) => (
                <div key={s.id} className="rounded-[12px] border border-[#EEEAF4] bg-[#FAFAFD] px-4 py-3">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="text-[15px] font-semibold text-[#0A0A0A]">{s.serviceName}</div>
                        <div className="text-[12px] text-[#6F6C90]">{s.serviceType}</div>
                      </div>
                    </div>
        ...
        <Panel className="col-span-1 p-6">
          ...
          <div className="mt-6">
            {mobileUsage.length === 0 ? (
              <div className="text-gray-500">No mobile subscriptions found.</div>
            ) : (
              (() => {
                ...
                return (
                  <>
                    <div className="mb-2 text-[14px] text-[#0A0A0A]">{used}{allowance ? `GB used` : ''}</div>
                    <Progress value={Math.min(used, allowance || used)} max={allowance || Math.max(used, 1)} />
                    {allowance ? (
                      <div className="mt-3 text-right text-[12px] text-[#6F6C90]">{allowance}GB total</div>
                    ) : null}
                    {m.lastUsageUpdate ? (
                      <div className="mt-3 text-[13px] text-[#6F6C90]">Last update: {parseAndFormatDate(m.lastUsageUpdate)}</div>
                    ) : null}
                  </>
                );
              })()
        ...
        <Panel className="col-span-1 p-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => router.push('/dashboard/billing')} className="rounded-[10px] bg-[#3F205F] px-3 py-1.5 text-[12px] font-semibold text-white">View All Bills</button>
          </div>
          <div className="mt-4 divide-y divide-[#EEEAF4]">
            {upcomingBills.length === 0 ? (
              ...
              upcomingBills.map((b: any) => (
                <div key={b._id || b.invoiceNumber} className="flex items-center justify-between py-3">
                  <div className="flex items-center gap-3">
                    <span className="grid h-8 w-8 place-items-center rounded-full bg-[#3F205F] text-white">
                    ...
                    <div>
                      <div className="text-[14px] font-semibold text-[#0A0A0A]">{b.invoiceNumber || 'Invoice'}</div>
                      <div className="text-[12px] text-[#6F6C90]">Due: {parseAndFormatDate(b.dueDate || b.due)}</div>
                    </div>
                  </div>
                  <div className="text-[14px] font-semibold text-[#0A0A0A]">{b.total ? `$${Number(b.total).toFixed(2)}` : '-'}</div>
                </div>
              ))
            )}
          </div>
        ...
        <Panel className="col-span-1 p-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => router.push('/dashboard/tickets')} className="rounded-[10px] border border-[#D9D4E5] px-3 py-1.5 text-[12px] font-semibold text-[#3F205F]">View All Tickets</button>
          </div>
          <div className="mt-4 space-y-3">
            {recentTickets.length === 0 ? (
              ...
              recentTickets.map((t: any) => (
                <TicketRow key={t._id} title={t.subject} id={`#T-${t._id.slice(-4)} • ${parseAndFormatDate(t.createdAt)}`} state={t.status === 'Resolved' ? 'Resolved' : 'Open'} />
              ))
            )}
          </div>
        </Panel>
        ...
        <Panel className="col-span-1 p-6">
          <div className="flex items-center justify-between">
            ...
            <button onClick={() => router.push('/dashboard/billing')} className="rounded-[10px] border border-[#D9D4E5] px-3 py-1.5 text-[12px] font-semibold text-[#3F205F]">View All</button>
          </div>
          <div className="mt-4 space-y-3">
            {recentInvoices.length === 0 ? (
              ...
              recentInvoices.map((inv: any) => (
                <InvoiceRow key={inv._id} date={parseAndFormatDate(inv.createdAt)} label={inv.invoiceNumber || 'Invoice'} amount={`$${Number(inv.total || 0).toFixed(2)}`} />
              ))
            )}
          </div>
        </Panel>
        ...
        <Panel className="col-span-1 p-6">
          ...
          <div className="mt-4 space-y-3">
            {quickActions.map((a: any) => (
              <button key={a.label} onClick={() => router.push(a.path)} className="flex w-full items-center gap-3 rounded-[12px] bg-[#F7F7FA] px-4 py-3 text-left text-[14px] font-semibold text-[#0A0A0A] hover:bg-[#EFEAF7]">
                {a.label}
              </button>
            ))}
          </div>
